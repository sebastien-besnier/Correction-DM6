...
# Il s'agit du polynome X+2X**2+3X**3
...
def derivative(P):
    ...
    Exemples:
        ...
        15*X**2 - 6
    """
    P_derivation=Polynome([0])
    for i in range(P.deg):
        P_derivaton[i]=(i+1)*P[i+1]
    return P_derivation
...
def nb_change_sign_at(polys, x):
    ...
    while i<(len(polys)-2):
        ...
        while j<len(polys) and eval_poly(polys[j],x)*eval_poly(polys[i],x)>=0:
            print eval_poly(polys[j],x)*eval_poly(polys[i],x)
            j+=1
            print "j=",j
        if j<len(polys):
            changement+=1
        ...
        i=j
        print "i=",i
    if eval_poly(polys[i],x)*polys[len(polys)-1]<0:
        print eval_poly(polys[i],x)*polys[len(polys)-1]        
        chgmt+=1
    return changementt    
