...
"Le terme constant de P est P[0]"

# 4. Calculer le reste de la division de 5X^42 + 3X+1 par 42 X^12 +3X-2.
A=Polynome([1,3]+40*[0]+[5])
B=Polynome([-2,3]+10*[0]+[42])
reste=A%B
...
## On remarque que P[i] renvoie fraction (p,1) donc on utilise int
...
def find_root(P, a, b, eps):
    ...
    while fin-deb>eps:
        m=(fin-deb)//2
        if -eps<eval_poly(P, m)<eps:
            return m
        if eval_poly(P, m)>eps:
            deb+=1
        if eval_poly(P, m)<eps:
            fin-=1    
...
def isolate_roots(P):
    ...
    """
    #la technique utilisée approche également les racines d'une autre manière 
...
P. n_prev contient le nombre de racines de P entre a et b."""
    ...
    #même chose dans la suivante sauf que les bornes sont a et b au lieu de 
    # -M et M    ????
