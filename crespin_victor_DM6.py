...
Le polynome construit est 3*X**3 + 2*X**2 + X.
...
X = Polynome ([-2, 3])
X[12] = 42
P%X     #-5/2744*X**9 + 5/1372*X**8 - 5/2058*X**7 + 5/9261*X**6 + 3*X + 1
...
def eval_poly(P, x):
    n=P.deg
    res=0
    for i in range (n,-1,-1):       #Boucle en partant du degrès de P jusqu'a
        res=res+P[i]*x**i           #0 avec un pas de -1.
    return res

#eval_poly(Polynome([1, 4, 5]),2), la fonction renvoie 29.
#eval_poly(Polynome([1, 23, 0]),0), la fonction renvoie bien 1.
...
#derivative(Polynome([6, 3, 9, 3])), la fonction renvoie 9*X**2 + 18*X + 3.
#derivative(Polynome([0])), la fonction renvoie 0.

def sturm_sequence(P):
    S=[P, derivative(P)]
    x=0
    while S[x+1].deg>=1:
        S.append(-S[x]%S[x+1])     #On ajoute à a liste "Suite de Sturm" l'opposé 
        x=x+1                      #du reste de la division euclidienne de P(x) 
    return S                       #par P(x+1)
